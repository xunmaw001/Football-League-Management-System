...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑购物商城</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>购物商城管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑购物商城</span></li>
			</ol>
		</div>
			...
			<form id="addOrUpdateForm">
				...
				<div class="form-item-file" >
					...
					<div class="upload btn-file">
						插入图片
						<input id="shangpinjianjieupload" name="file" type="file">
					</div>
					<script id="shangpinjianjieEditor" type="text/plain"></script>
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "gouwushangcheng/info/" + id,
		...
		function upload(){
			...
			$('#shangpinjianjieupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('shangpinjianjieEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("gouwushangcheng/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("gouwushangcheng/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function setContent(){
			if(ruleForm.shangpinjianjie != null && ruleForm.shangpinjianjie != 'null' && ruleForm.shangpinjianjie != ''){
				var shangpinjianjieMes = '' + ruleForm.shangpinjianjie;
				var shangpinjianjieUeditor = UE.getEditor('shangpinjianjieEditor');
				shangpinjianjieUeditor.ready(function() {
					shangpinjianjieUeditor.setContent(shangpinjianjieMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('shangpinjianjieEditor').hasContents()){
				$('#shangpinjianjie-input').attr('value',UE.getEditor('shangpinjianjieEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("gouwushangcheng/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("gouwushangcheng"+colName,"GET",{
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.shangpinjianjie != null && res.data.shangpinjianjie != ''){
                        ...
                        var shangpinjianjieMes = '' + res.data.shangpinjianjie;
                        var shangpinjianjieUeditor = UE.getEditor('shangpinjianjieEditor');
                        shangpinjianjieUeditor.ready(function() {
                            shangpinjianjieUeditor.setContent(shangpinjianjieMes);
                        });
